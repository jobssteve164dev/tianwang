...
router.post('/threat', async (req, res) => {
  try {
    ...
    if (!(await accessibleAgentIds(req)).includes(agent_id)) {
      return res.status(403).json({ success: false, error: '无权访问此设备' });
    }
    ...
    // 通过WebSocket广播新告警
    if (req.app.get('io')) {
      req.app.get('io').to(`threats:${req.organizationId || null}`).emit('new-alert', {
        id: alert.id,
        title: alert.title,
        severity: alert.severity,
        deviceId: alert.deviceId,
