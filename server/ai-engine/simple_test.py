...
def test_file_structure():
    ...
    for directory in directories:
        assert os.path.isdir(directory), f"{directory} 目录缺失"
    ...
    for file in files:
        assert os.path.isfile(file), f"{file} 文件缺失"
        print(f"✓ {file} 文件存在")
...
def test_basic_imports():
    ...
    try:
        ...
        assert json.loads(json_str)["system"]["cpu_usage"] == 75.0

    except Exception as e:
        print(f"✗ 基本导入测试失败: {e}")
        raise AssertionError(f"基本导入测试失败: {e}") from e
...
def test_file_content():
    ...
    try:
        ...
        with open('src/config.py', 'r', encoding='utf-8') as f:
            ...
            assert 'AIEngineConfig' in config_content
        ...
        with open('src/services/ai_service.py', 'r', encoding='utf-8') as f:
            ...
            assert 'class AIService' in ai_service_content
            print("✓ AI服务文件包含AIService类")

    except Exception as e:
        print(f"✗ 文件内容测试失败: {e}")
        raise AssertionError(f"文件内容测试失败: {e}") from e
