...
class RuleEngine:
    ...
    def __init__(self):
        ...
        for rule_type in self.rules.keys():
            os.makedirs(os.path.join(self.rules_dir, rule_type), exist_ok=True)
    ...
    async def initialize(self):
        ...
        try:
            ...
            # 加载本地规则
            await self._load_local_rules()
            
            # 更新规则库
            await self._update_rules()
            
            self.is_initialized = True
            logger.info(f"规则引擎初始化完成，共加载 {self.metrics['rules_loaded']} 条规则")
...
'''
    ...
    async def match_rules(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            # 匹配Sigma规则
            sigma_matches = await self._match_sigma_rules(data)
            matches.extend(sigma_matches)
            
            # 匹配YARA规则
            yara_matches = await self._match_yara_rules(data)
            matches.extend(yara_matches)
            
            # 匹配网络规则
            network_matches = await self._match_network_rules(data)
    ...
    def is_healthy(self) -> bool:
        """检查服务健康状态"""
        return self.is_initialized and self.metrics["rules_loaded"] > 0
    
    def get_metrics(self) -> Dict[str, Any]:
        """获取服务指标"""
        return {
            "service_status": "healthy" if self.is_healthy() else "unhealthy",
            "rules_by_type": {rule_type: len(rules) for rule_type, rules in self.rules.items()},
